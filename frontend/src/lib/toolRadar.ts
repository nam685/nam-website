...
/** Group tools by status for the tab strip; within a group, new/unreviewed feed entries surface first. */
export function groupToolsByStatus(
  ...
  for (const status of TOOL_TABS) {
    groups[status].sort((a, b) => Number(b.is_new) - Number(a.is_new));
  }
  return groups;
}
