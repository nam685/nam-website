...
### Home
The landing page is a constellation: each section is an equally-sized glowing dot orbiting a center, with a pill tooltip on hover. Moving the mouse sweeps an ambient background glow through the dots' rainbow of hues. At the center sits a circular profile photo of Nam — one of a small rotating set, chosen at random on each visit — whose rim and outer edge are tinted live with the same hue the cursor is pointing toward, so the portrait picks up the color of whichever section you're drifting toward.

Behind it all, an animated **particle field**: glowing comets launch from the photo's edge, walk outward past the dots along a stepped radial-then-circular path, and leave faint traces that fade over time. A cursor "spotlight" — round near the center, stretching into a radial beam toward the edges — brightens whichever traces it sweeps over. The traces and spotlight follow the same cursor-driven hue as the photo rim; the section dots keep their own colors. The field is purely decorative and honours `prefers-reduced-motion` (it doesn't animate when reduced motion is requested).
...
### Listens
A public, interactive **graph** of Nam's music, built from YouTube Music history. Tracks, artists, albums, and genre **tags** are nodes. Edges come in three layers: a **structural** backbone (track↔artist↔album), a **tag/genre** connective layer (from Last.fm `artist.getTopTags`) whose few, widely-shared tag nodes keep the whole graph connected so nothing strands as an island, and a single **affinity** layer of recommendation edges. Affinity blends three signals — Nam's own co-listen habits (personal collaborative filtering), Last.fm `artist.getSimilar`/`track.getSimilar` (global collaborative filtering), and content-based genre-tag overlap between artists — each rescaled with **Mutual Proximity** (Flexer & Stevens) so the mutually-nearest pairs survive and the geometric "hub" artifacts that used to dominate are pruned. Each visit reveals a **patch** — a seed node and its neighborhood — and the "↻ New Patch" button surfaces a fresh region. A search box jumps to any artist/track/album, and clicking a node re-centers the graph to walk outward. Node size reflects play count; liked nodes show a yellow ring and subscribed artists a dashed ring. Admin users can sync new data, import Google Takeout history, and play tracks (and artist/album top tracks) via the embedded mini music player that persists site-wide with queue management, shuffle, and repeat controls; the Sync and re-auth controls are a single button that flips into a re-auth action when the YouTube Music session expires. The player supports an endless **radio** mode (∞ toggle): when enabled, it keeps the queue full by auto-playing tracks related to the current song. Both shuffle and radio are the same primitive — a **uniform random walk** over the graph. Because the graph is de-hubbed, a plain random walk visits nodes in proportion to their degree, so popular/central tracks recur naturally without any explicit popularity weighting. Each step also has a small **ε-teleport** probability of jumping to a uniform-random track anywhere in the graph (the PageRank random-surfer), which gives even stray lone songs in disconnected islands a nonzero chance to surface, so nothing is permanently stranded. Admins also get a diagnostic full-graph visualization at **`/listens/graph`**, reachable via an admin-only **`⊹ FULL GRAPH`** button on `/listens` (color by connected component, size by degree, edge-type filters, and a panel of connectivity stats — components, degree histogram, top hubs, islands, and internal bridge/articulation points) for auditing graph health. It renders fast when zoomed out (a minimal flat-dot mode with edges hidden), restoring full detail on zoom-in, and is served from a cache warmed on each graph rebuild.
...
### Plays — Chess, AoE 2 & Badminton
The plays page has three top-level, game-namespaced sections reflected in the URL path: **Chess** (`/plays/chess`), **AoE 2** (`/plays/aoe2`) and **Badminton** (`/plays/badminton`). The bare `/plays` redirects to `/plays/chess`, and the full-width selector at the top switches between them. (Old deep links keep working — see below.)
...
- **Army & Stats** — the build-order classification (matched/missed signals) at the **top**, then **one unified full-width production graph** that combines the army and the tech timeline in a single view: above the x-axis a stacked-area of villagers + army units by type (cumulative *produced* — queued upper bounds, never live counts) with a vertical legend on the right (swatch + icon + name + total); m:ss time labels in their own band below the axis; and below zero an **upgrade/tech research icon row** plotting eco/military/university techs over time as their real icons (the units are already the areas, so the negative band adds the *other* dimension — what was researched, and when). Feudal/Castle/Imperial age-up guide-lines + age icons span the chart. An efficiency panel rounds it out: TC idle as a pre-cap % ("idle before 200 pop, age-ups excluded"), longest villager gap, and a clear APM split ("APM N — X eco · Y military · Z other", captioned "commands per minute, by what they controlled"). All AoE2 DE icons are bundled locally (same-origin, named by aoe2techtree *picture_index*); every unit/building/tech/age name in the coach's vocabulary resolves to a real icon (100% coverage, verified by a test), and a genuinely-unknown name (e.g. a brand-new unit not yet in the coach data) falls back to a "?" chip rather than a broken image.
...
#### Badminton — clear swing analysis (`/plays/badminton`)
Coach-style analysis of the forehand overhead **clear** for the owner and friends. Anyone can click **+ analyse my clear**, say who they are (pick an existing player or type a new name), optionally give their height and racket hand, the recording date (defaults to the file's date) and the report language (English / Deutsch / Tiếng Việt), and upload one practice video (up to 2 GB, sent in 8 MB chunks with a progress bar; filming tips and a privacy note sit next to the form). Nothing runs until the admin watches the upload in the admin **queue** and clicks **approve & run now** (or rejects it). The analysis itself runs on the owner's GPU PC: `scripts/badminton_worker.py` picks up approved uploads, feeds them to the separate `badminton-coach` project (pose/racket/shuttle tracking + a Claude coach), mirrors progress back, and publishes the finished report. Uploaders follow their uploads under **your uploads** (waiting for approval → queued → analysing with a progress bar → report ready).

Reports are public and laid out like a wiki: a **left nav tree** (players → dated sessions → attempts, with expand/collapse chevrons) and a **document pane** on the right. Clicking a player expands their sessions; clicking a session opens the coach's verdict (progress since last time, what's working, then up to four prioritised findings with cue, drill and folded evidence whose attempt numbers open that attempt) and expands its attempts in the tree; clicking an attempt opens that attempt's page — its one-line verdict and annotated slow-motion clip first, with the raw clip, key-moment grid, motion signals, measurements vs the reference player, footwork and notes collapsed — plus prev/next buttons. On mobile the tree stacks above the document. Deep link: `/plays/badminton?player=<slug>&s=<id>&a=<attempt>`. The page has its own description and Open Graph tags for link previews, is listed in `/sitemap.xml`, and has a project card on `/codes`. The raw upload is private (unguessable URL, admin + worker only) and deleted once the report is in; the admin can delete any report.
### Watches
