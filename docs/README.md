...
**AoE 2 tab** showcases the owner's Age of Empires II: Definitive Edition 1v1 ranked games. A stats header shows the current ELO, win/loss record, total games played, and top civilization. Below the header, games are listed in a single-selection accordion — only one game expands at a time; selecting a different game collapses the previous one. When a game is expanded, its optional highlight-clip `<iframe>` mounts; collapsed rows mount nothing. Each expanded game displays per-game metrics parsed from the recorded file: Feudal/Castle/Imperial age uptimes, build order, economy-tech timing, army composition, and APM, plus an opening classification. There is no idle-TC stat. Chat and player names are stripped during parsing for privacy; the opponent is identified by civilization only. A share link is available via the ⋮ menu on the expanded game; visiting `/plays?game=<id>` deep-links directly to the AoE 2 tab with that game expanded. Games are ingested automatically: a local watcher (`scripts/aoe2_watcher.py`) detects new `.aoe2record` files and uploads them to the admin-only `/api/aoe2/upload/` endpoint; non-1v1, vs-AI, and single-player recordings are skipped and not shown.
