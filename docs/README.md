...
- **Army & Stats** — the army *produced* (cumulative-queued upper bounds, never live counts), villagers produced + a villager-count curve, an efficiency panel (TC idle as a pre-cap % "idle before 200 pop, age-ups excluded", longest villager gap, APM eco/military split), and the build-order classification with matched/missed signals. Engine-only stats (units killed/lost, live army size, % map explored, resources collected) are shown as honest "unavailable" placeholders.
- **Technology** — the event timeline (age arrivals, eco/military/university tech, production milestones) drawn with **real AoE2 DE icons** (bundled locally, same-origin; named by aoe2techtree *picture_index*), plus per-tech timing columns. Names without a bundled icon fall back to a clean monogram glyph.
