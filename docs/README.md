...
### Plays — Chess & AoE 2
The plays page has two top-level tabs: **Chess** and **AoE 2**.
...
**AoE 2 tab** showcases the owner's Age of Empires II: Definitive Edition 1v1 games. A stats header shows the current ELO, win/loss record, total games played, and top civilization. Below the header, games are listed in a single-selection accordion — only one game expands at a time; selecting a different game collapses the previous one. When a game is expanded, its optional highlight-clip `<iframe>` mounts; collapsed rows mount nothing. Each expanded game displays per-game metrics parsed from the recorded file: Feudal/Castle/Imperial age uptimes, build order, economy-tech timing, army composition, and APM, plus an opening classification.

Expanded games also render a CaptureAge-style match-analysis view built from the `aoe2coach` v2 reconstruction (you = blue, opponent = red):
- a **strategic-map minimap** drawn from exact building coordinates (base centroids, walls, forward buildings ringed, and zone-pinned engagement markers sized by aggressive-command volume — placements, not what survived);
- a **build-order classification** with 1–3 ranked candidates, each with a confidence score and matched/missed signals;
- a **timeline** of age arrivals (guide-lines), eco/military/university tech, and production milestones;
- an **efficiency** panel (TC idle, longest villager gap, and an APM eco/military split bar);
- a **mistakes** list from deterministic checks — an empty list honestly reads "no mistakes detected" — each flagged item carrying a severity, a confidence tier, a fix, and a "learn more" deep-link to study material;
- an **economy** estimate (Tier-B) showing per-age villager allocation, clearly badged `~est`; collected totals are shown only when the model does not self-suppress them, otherwise a qualitative shape;
- a **produced-counts** strip (army/villagers *produced* — cumulative-queued upper bounds, never live counts), with engine-only stats (units killed, live army, % map explored, resources collected) shown as honest "unavailable" placeholders.

Every panel is honest about its data provenance (exact = solid; estimate = dashed/`~est` badge; engine-only = "unavailable"). The coach commentary (a WHAT-HAPPENED summary + ANALYSIS, written by an agentic Claude coach over the reconstruction) renders alongside. Matches analyzed before the v2 upgrade have no reconstruction and gracefully fall back to the metric tiles + coach text. Chat and player names are stripped during parsing for privacy; the opponent is identified by civilization only. A share link is available via the ⋮ menu on the expanded game; visiting `/plays?game=<id>` deep-links directly to the AoE 2 tab with that game expanded. Games are ingested automatically: a local watcher (`scripts/aoe2_watcher.py`) detects new `.aoe2record` files and uploads them to the admin-only `/api/aoe2/upload/` endpoint; non-1v1, vs-AI, and single-player recordings are skipped and not shown.
